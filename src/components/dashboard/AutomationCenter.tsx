            ...
            {staleTasks.length > 0 ? (
              staleTasks.map((task) => (
                <div key={task.id} className="p-3 bg-gray-50 rounded-lg flex justify-between items-center border border-gray-100 transition-colors hover:bg-gray-100">
                  <span className="font-medium text-gray-800 text-sm truncate max-w-[150px] sm:max-w-none">{task.title}</span>
                  <span className="text-xs text-gray-500 bg-white px-2 py-1 rounded-md border border-gray-200 flex-shrink-0">
                    Last update {task.daysStale}d ago
                  </span>
                </div>
              ))
            ) : (
              ...
              riskTasks.map((task) => (
                <div key={task.id} className={`p-3 rounded-lg flex justify-between items-center border transition-colors ${task.severity === 'high' ? 'bg-red-50 border-red-100 hover:bg-red-100' : 'bg-orange-50 border-orange-100 hover:bg-orange-100'}`}>
                  <div className="min-w-0 flex-1 mr-2">
                    <div className={`font-medium text-sm truncate ${task.severity === 'high' ? 'text-red-900' : 'text-orange-900'}`}>{task.title}</div>
                    <div className={`text-xs mt-0.5 ${task.severity === 'high' ? 'text-red-600' : 'text-orange-600'}`}>
                      {task.progress}% Complete
                    </div>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded-md border font-medium flex-shrink-0 ${task.severity === 'high' ? 'bg-red-100 border-red-200 text-red-700' : 'bg-orange-100 border-orange-200 text-orange-700'}`}>
            ...
            {achievedMilestones.length > 0 ? (
              achievedMilestones.map((milestone) => (
                <div key={milestone._id} className="p-3 bg-green-50 rounded-lg flex justify-between items-center border border-green-100 transition-colors hover:bg-green-100">
                  <span className="font-medium text-green-900 text-sm truncate max-w-[150px] sm:max-w-none">{milestone.taskTitle}</span>
                  <span className="text-xs text-green-600 bg-green-100 px-2 py-1 rounded-md border border-green-200 flex items-center gap-1 font-medium flex-shrink-0">
